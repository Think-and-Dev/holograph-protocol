...
contract CountdownERC721PurchaseTest is CountdownERC721Fixture, ICustomERC721Errors {
  ...
  function test_PurchaseWrongPrice() public setupTestCountdownErc721(DEFAULT_MAX_SUPPLY) setUpPurchase {
    /* -------------------------------- Purchase -------------------------------- */
  ...
  function test_GetContractURI() public setupTestCountdownErc721(DEFAULT_MAX_SUPPLY) setUpPurchase {
    ...
    assertEq(countdownErc721.contractURI(), expectedURI);
  }

  function test_SetContractURI() public setupTestCountdownErc721(DEFAULT_MAX_SUPPLY) setUpPurchase {
    string memory expectedURI = "https://example.com/new-metadata.json";

    vm.prank(DEFAULT_OWNER_ADDRESS);
    vm.recordLogs();

    countdownErc721.setContractURI(expectedURI);
    assertEq(countdownErc721.contractURI(), expectedURI);
  ...
  function test_GetSourceChainPrepend() public setupTestCountdownErc721(DEFAULT_MAX_SUPPLY) setUpPurchase {
